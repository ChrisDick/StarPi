...
{
    CC_TIME_T RA_time = { 14, 16, 22.1 };
    double Dec = ((14/24) + ((6/60)/24) + ((6.6/3600)/24))*(2*M_PI);

    double Pitch = HalAccelerometer::Accelerometer.HalAccelerometerGetPitch();
    double Roll = HalAccelerometer::Accelerometer.HalAccelerometerGetRoll();
    //double Heading = 0;
    ...
    Angles.Azimuth = HalCompass::Compass.HalCompassGetTiltCompensatedHeading();
    if ( Angles.Azimuth < 0 )
    {
        Angles.Azimuth += (2* M_PI );
    }
